...
-- Composite index for daily aggregation
CREATE INDEX IF NOT EXISTS idx_api_usage_service_date 
    ON api_usage(service, DATE(recorded_at));
...
FROM api_usage
WHERE success = true
GROUP BY service, brand_id, campaign_id, DATE(recorded_at);

-- Indexes on materialized view
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_daily_usage_pk 
    ON mv_daily_usage(service, COALESCE(brand_id::text, ''), COALESCE(campaign_id::text, ''), usage_date);
...
BEGIN
    ...
    SELECT 
        au.service,
        DATE(au.recorded_at) as usage_date,
        COUNT(*) as call_count,
    ...
    GROUP BY au.service, DATE(au.recorded_at)
    ORDER BY DATE(au.recorded_at) DESC, au.service;
END;
$$;
...
EXCEPTION WHEN OTHERS THEN
    -- Fall back to non-concurrent if unique index doesn't exist
    REFRESH MATERIALIZED VIEW mv_daily_usage;
    
    RETURN jsonb_build_object(
        'success', true,
        'refreshed_at', NOW(),
        'note', 'non-concurrent refresh'
    );
END;
$$;
