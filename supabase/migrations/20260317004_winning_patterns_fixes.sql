-- =============================================================================
-- Migration: 20260317004_winning_patterns_fixes.sql
-- Phase: Roadmap #20 — Fix recompute_winning_patterns bugs
-- =============================================================================
-- Fixes:
--   1. Ambiguous column reference in recompute_winning_patterns
--      (RETURNS TABLE outputs clash with table columns)
--   2. NULL-safe UNIQUE index for winning_metadata_patterns
--      (vibe_preset IS NULL rows can't ON CONFLICT with standard UNIQUE)
-- =============================================================================

-- =====================================================
-- 1. Replace the UNIQUE constraint with a NULL-safe index
--    Standard UNIQUE treats NULL != NULL, so UPSERT fails
--    when vibe_preset IS NULL
-- =====================================================

-- Drop the old constraint (constraint name auto-generated from CREATE TABLE)
ALTER TABLE winning_metadata_patterns
    DROP CONSTRAINT IF EXISTS winning_metadata_patterns_brand_id_platform_vibe_preset_windo_key;

-- Create NULL-safe unique index using COALESCE
CREATE UNIQUE INDEX IF NOT EXISTS idx_wmp_unique_brand_plat_vibe_window
    ON winning_metadata_patterns (
        brand_id, platform, COALESCE(vibe_preset, ''), window_days
    );

-- =====================================================
-- 2. Fix recompute_winning_patterns
--    Add #variable_conflict use_column directive to resolve
--    ambiguity between RETURNS TABLE output params and
--    table column names
-- =====================================================
