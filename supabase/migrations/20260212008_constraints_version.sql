-- =====================================================
-- CONSTRAINTS VERSION TRACKING
-- Migration: 20260212008_constraints_version.sql
--
-- Tracks which constraint rule-set was used to validate
-- AI-generated metadata, so we can identify stale metadata
-- when constraints are updated later.
--
-- Date: February 12, 2026
-- =====================================================

-- 1. Add constraints_version column
ALTER TABLE post_metadata ADD COLUMN IF NOT EXISTS constraints_version INTEGER DEFAULT 1;

-- 2. Update upsert to accept and store constraints_version
DROP FUNCTION IF EXISTS upsert_post_metadata(UUID, TEXT, JSONB, TEXT, TEXT, TEXT, TEXT, INTEGER);
...
CREATE OR REPLACE FUNCTION upsert_post_metadata(
    ...
    p_constraints_version  INTEGER DEFAULT 1
)
RETURNS UUID
...
BEGIN
    ...
    INSERT INTO post_metadata (
        ...
        generated_by, worker_id, schema_version, constraints_version,
    ...
    VALUES (
        ...
        'ready', now(), 1,
        p_generated_by, p_worker_id, p_schema_version, p_constraints_version,
        NULL, NULL, NULL
    )
    ON CONFLICT (post_id, platform) DO UPDATE SET
        ...
        constraints_version = EXCLUDED.constraints_version
...
-- 3. Update get_post_metadata to include constraints_version
DROP FUNCTION IF EXISTS get_post_metadata(UUID, TEXT);
...
GRANT EXECUTE ON FUNCTION upsert_post_metadata(UUID, TEXT, JSONB, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER) TO service_role;
GRANT EXECUTE ON FUNCTION upsert_post_metadata(UUID, TEXT, JSONB, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER) TO authenticated;
