-- Migration: Redo Completed Job
-- Allows re-running a completed (or any non-active) job from scratch.
-- Resets the job to pending, clears step states and generated assets,
-- and resets associated posts so the full pipeline re-runs.
...
BEGIN
    ...
    UPDATE jobs SET
        ...
        attempt_count = 0,
        meta = jsonb_set(
            -- Clear step states but preserve other meta (vibe_preset, platforms, etc.)
    ...
    --    (images, audio, video, subtitles, etc.)
