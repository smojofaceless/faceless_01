...
The Caption/Tags Learning Loop stores metadata versions, correlates them with engagement metrics, and biases future AI generation toward high-performing patterns. It also enables A/B variant testing at the job level.

**Level 3 scope** — no ML models, no auto-optimization engines. The system captures data, surfaces performance, and lets the generator fetch exemplars; humans decide when to intervene.
