...
| Mar 1, 2026 | 4.6 | **TTS Truncation Auto-Retry + Video Cutoff Fix + SVC_ROLE_KEY Deployment**: (1) **TTS truncation retry** — `executeVoiceStepOpenAI()` now detects when OpenAI TTS silently truncates trailing words (via forced-alignment analysis of consecutive 0.08s interpolated words). On detection, retries up to 2×: first with `speed: 0.95`, then by splitting text at sentence boundary and concatenating two audio clips. `generateSplitTTS()` helper + `findSentenceSplitPoint()` for natural text splitting. Job meta tracks `tts_retry_attempts`, `tts_retry_strategy`, `tts_truncation_resolved`. (2) **Video cutoff fix** — `Math.floor` → `Math.ceil` for Ken Burns + effects frame calculations in `server_clean.js` (4 locations). Removed `-shortest` flag from `addAudioToVideo` FFmpeg command. Added 0.5s buffer to last scene in `createVideoFromImages`. (3) **SVC_ROLE_KEY fallback** deployed to ALL 9 edge functions: `metadata-scheduler`, `generate-post-metadata`, `post-worker`, `schedule-posts`, `create-job`, `check-job`, `test-scenes`, `run-job`, `worker-v1`. All now use `Deno.env.get("SVC_ROLE_KEY") \|\| Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")`. Root cause of missing post tags: stale auto-injected JWT caused metadata-scheduler/generate-post-metadata to fail silently → no `post_metadata` records → post-worker used fallback path with 0 tags. (4) **Max attempt safeguard** — `find_eligible_jobs` SQL caps at 10 attempts, `sweep_stuck_jobs` function added. |
...
| **Max Attempt Safeguard** | Mar 1, 2026 | `find_eligible_jobs` SQL now caps at 10 attempts. New `sweep_stuck_jobs` function marks permanently stuck jobs as failed. Migration: `20260327001_job_max_attempts_safeguard.sql`. |
...
> **Status:** 🟡 Foundation laid (February 19, 2026), enhanced Feb 23, 2026
